...
data_clean <- data %>%
  filter(Sample_Number %in% c(21, 6, 27, 38)) %>%
...
ggplot(data_clean, aes(x = Time, y = Height, group = interaction(Accession, treat), color = treat)) +
  ...
  scale_y_continuous(limits = c(NA, max(data_clean$Height) + 10)) + # Extend y-axis range
  facet_wrap(~ Accession, scales = 'fixed') +
...
ggsave("PostFrostHeightByAccession_17x17cm_600dpi.jpg", units = "cm", width = 17, height = 17, dpi = 600)
